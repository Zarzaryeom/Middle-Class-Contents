...
public class ThreadTest02 {
	...
	public static void main(String[] args) {
		...
		Thread th2 = new Thread(r1);  // Thread클래스를 생성할 때 생성자에 구현 class를 넣어서 생성한다.
		th2.start();
		
		
		// 3) Runnable인터페이스를 익명구현체로 구현하여 사용하는 방법, 일회성으로 사용하는 방법이다.
...
class MyThread1 extends Thread{
	@Override
	public void run() {
		// 이 run()메서드에는 이 쓰레드가 해야할 일을 코당하면 된다.
		for(int i = 1; i <= 200; i++) {
			System.out.print("*");
			try {
				// Thread.sleep(시간) 메서드는 주어진 '시간'동안 작업을 잠시 멈춘다.
