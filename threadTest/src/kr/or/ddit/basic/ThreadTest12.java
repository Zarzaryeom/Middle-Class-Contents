...
/*
	3개의 스레드가 각각 알파벳을 A~Z까지 출력하는데 출력이 끝낸 순서대로
	결과를 나타내는 프로그램 작성하기.
*/
...
class DisplayCharacter extends Thread{
	public static String setRank = ""; // 출력을 끝낸 순서대로 이름이 추가될 변수
	public String name;
	...
	public void run() {
		for(char ch = 'A'; ch <= 'Z'; ch++) {
			...
			try {
				// 일시정지값을 난수로 지정한다.(201 ~ 800 사이)
				Thread.sleep((int)Math.random() * 600 + 201);
			} catch (InterruptedException e) {
			}
		}
