 ...
 * UDP 방식 
 *  - 특징
 *    ▷ 비연결 지향(연결이 안되어 있어도 데이터를 보낼 수 있다.)
 *    ▷ 신뢰성이 없다
	...
	public static void main(String[] args) {
		try {
			...
			while(true) {
				...
				// 수신용 패킷 객체 생성
				// DatagramPacket 파라미터에 들어갈 내용 ==> 데이터가 저장될 byte형 배열, 배열의 길이
				inpacket = new DatagramPacket(bMsg, bMsg.length);
				
				// 데이터 수신하기 ==> receive()메서드 이용
				// receive()메서드는 데이터가 올 때까지 기다린다.
				// 수신된 데이터의 패킷정보는 지정한 패킷객체변수(inpacket)에 저장된다. 
				...
				// 상대방이 보낸 메세지 출력하기 -> 실제 가져온 데이터 갯수만큼 출력해야한다. 그렇지 않으면 512개 갯수에서 남은 만큼 공백 출력
				// inpacket.getLength() => 실제 읽어온 데이터의 길이 반환
				// inpacket.getData()
				//		=> 실제 읽어온 데이터를 byte배열로 반환한다.
				//		=> 수신용 패킷에 설정한 byte배열과 같다.
				
				// byte형 배열의 데이터를 문자열로 변환하기(어떤 배열의, 이곳을 첫번쨰로, 이정도 길이만큼, 인코딩 형식)
				...
				// 상대방에게 메세지 보내기 (수신받은 데이터 그대로 송신하기)
				
				// 송신할 메세지를 byte배열로 변환한다.
				byte[] sendMsg = msg.getBytes("utf-8");
				
				// 송신용 패킷객체 생성 
				//   => 전송할 데이터가 저장된 byte배열, 전송할 자료의 길이(배열의 길이), 수신할 상대방 주소 정보, 포트번호를 지정하여 DatagramPacket객체를 생성한다.				
