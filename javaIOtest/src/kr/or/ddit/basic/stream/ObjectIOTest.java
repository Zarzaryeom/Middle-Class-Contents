...
public class ObjectIOTest {
	...
	public static void main(String[] args) {
		...
		} catch(EOFException e) { // 읽기 작업이 끝나고 나온 오류를 넘기기 위한 명령어, while문을 끝내고는 바로 이곳으로 넘어 오게 된다.
			System.out.println("객체 읽기 작업 끝");
		} catch (IOException e) {
			e.printStackTrace();
		} catch (ClassNotFoundException e) {
			e.printStackTrace();
		} catch (Exception e){ // 오류를 확인하기 위한 명령어
			e.printStackTrace();
		} finally {
			if(oin != null)	try { oin.close(); } catch (IOException e) { } //oin을 닫기 위해서는 이곳에서 처리
		}
...
class Member implements Serializable{
	private static final long serialVersionUID = -3682397840947077249L; // 버전을 비교할 수 있는 숫자 데이터
